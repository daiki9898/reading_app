...
postリクエスト時にcsrfトークンを送信しています。(th:action)
### 2. **CORS設定**
- 異なるオリジンでのリソース共有を防ぐため、ローカルホストからのリクエストしか受け付けていないようにしています。これはXSSやCSRF対策になります。<br>
ちなみにThymeleafのth:textは、入力され文字列をエスケープして表示しています。
