...
Thymeleafのth:actionを使用して、postリクエスト時にcsrfトークンを送信しています。
### 2. **CORS設定**
- 異なるオリジンでのリソース共有を防ぐため、ローカルホストからのリクエストしか受け付けていないようにしています。<br>これはXSSやCSRF対策になります。
- また、Thymeleafのth:textを使用して、入力された文字列をエスケープして表示しています。
